...
export default function FilterBar({ filters, neighborhoods, onChange, showArchivedStatus = false }: FilterBarProps) {
  const visibleStatuses = showArchivedStatus
    ? REPORT_STATUSES
    : REPORT_STATUSES.filter((s) => s !== "archived");
  ...
  const selectClass =
    "rounded-md border border-gray-300 bg-white px-3 py-1.5 text-sm text-gray-700 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500";
  ...
  return (
    <div className="flex flex-wrap gap-2 items-center">
      ...
      {(filters.category || filters.status || filters.severity || filters.neighborhood_id) && (
        <button
